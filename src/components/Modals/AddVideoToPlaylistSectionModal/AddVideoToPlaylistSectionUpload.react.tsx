...
const UploadProgressUI: React.FC<any> = props => {
  ...
  const generateImageFromVideo = video => {
...
const AddVideoToPlaylistSectionUpload: React.FC<any> = props => {
  ...
  const saveVideoToSection = async (
    ...
    const saveUploadedVideoResponse = await saveUploadedVideo(
      formValues,
      props.userId,
      uploadVideoResponse,
      100,
    );
    if (saveUploadedVideoResponse.error === null) {
      props.setSubmittedText(ProgressMessages.ADDING_TO_PLAYLIST);

      const saveFormDataResponse = await saveFormData(
        [saveUploadedVideoResponse.saveResourceResponse.objectID],
        props.playlistId,
        props.sectionIndex,
        props.userId,
      );

      if (saveFormDataResponse.errors && !saveFormDataResponse.errors.length) {
